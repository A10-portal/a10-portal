...
export default async function handler(req, res) {
    ...
    if (req.method === 'GET' && !action) {
        ...
        try {
            ...
            // Count referrals and balance
            let referralCount = 0;
            let referralBalance = user.referralBalance || 0;
            if (user.referralCode) {
                referralCount = await db.collection('users').countDocuments({ referredBy: user.referralCode });
            }
            ...
            return res.status(200).json({
                ...
                referralBalance: parseFloat(referralBalance) || 0
            });
        } catch (e) {
