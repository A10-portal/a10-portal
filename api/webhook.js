...
export default async function handler(req, res) {
    ...
    try {
        ...
        console.error('Failed to expand session:', e.message);
    }
    ...
    try {
        ...
        // Enrich items with CJ variant data
        const enrichedItems = await Promise.all(cartItems.map(async (item) => {
            ...
            // Parse name and variant — supports both formats:
            // "ProductName — Color / Size (xQty)" and "ProductName - VariantName (xQty)"
            ...
            // Always try to get SKU and confirm variant image from CJ
            try {
                ...
                if (variants.length > 0) {
                    ...
                    if (vidPart) {
                        ...
                        if (matched) {
                            ...
                            // Always prefer variant-specific image (shows correct color/variant)
            ...
            // item.price is already unit*qty total — use as-is
        ...
        // ── ADMIN EMAIL ───────────────────────────────────────────
        // Fetch image and return as base64 so Gmail embeds it directly
        ...
        const itemsAdminHtml = enrichedItems.map(i => {
            const imgCell = i.image
                ? `<img src="${proxyImg(i.image)}" width="70" height="70" style="object-fit:cover;display:block;border:1px solid #333">`
                : `<div style="width:70px;height:70px;background:#1a1a1a;border:1px solid #333;display:flex;align-items:center;justify-content:center;color:#555;font-size:10px">IMG</div>`;
            return `<tr>
                ...
                <td style="padding:12px;border-bottom:1px solid #222;vertical-align:top">
                    ...
                    <a href="https://cjdropshipping.com/product/${(i.pid||'').split('-')[0]}" style="font-size:11px;color:#4da6ff;margin-top:4px;display:inline-block">View on CJ →</a>
                </td>
        ...
        try {
            await resend.emails.send({
                ...
                <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:24px;background:#111;border:1px solid #222">
                    <tr><td style="padding:10px 14px;color:#888;width:160px;border-bottom:1px solid #1a1a1a">Order Date & Time</td><td style="padding:10px 14px;font-weight:700;color:#fafafa;border-bottom:1px solid #1a1a1a">${new Date().toLocaleString('en-US',{month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit',hour12:true,timeZone:'America/New_York'})} EST</td></tr>
<tr><td style="padding:10px 14px;color:#888;width:160px;border-bottom:1px solid #1a1a1a">Order ID (MongoDB)</td><td style="padding:10px 14px;font-weight:700;color:#fafafa;border-bottom:1px solid #1a1a1a;font-family:monospace">${orderId}</td></tr>
                    <tr><td style="padding:10px 14px;color:#888;border-bottom:1px solid #1a1a1a">Stripe Payment ID</td><td style="padding:10px 14px;color:#aaa;border-bottom:1px solid #1a1a1a;font-family:monospace">${paymentIntentId}</td></tr>
                    <tr><td style="padding:10px 14px;color:#888;border-bottom:1px solid #1a1a1a">Stripe Session ID</td><td style="padding:10px 14px;color:#aaa;border-bottom:1px solid #1a1a1a;font-family:monospace">${stripeSessionId}</td></tr>
                    <tr><td style="padding:10px 14px;color:#888;border-bottom:1px solid #1a1a1a">Customer Name</td><td style="padding:10px 14px;font-weight:700;color:#fafafa;border-bottom:1px solid #1a1a1a">${realName}</td></tr>
                    <tr><td style="padding:10px 14px;color:#888;border-bottom:1px solid #1a1a1a">Customer Email</td><td style="padding:10px 14px;color:#4da6ff;border-bottom:1px solid #1a1a1a">${customerEmail}</td></tr>
                    <tr><td style="padding:10px 14px;color:#888;border-bottom:1px solid #1a1a1a">Customer Phone</td><td style="padding:10px 14px;color:#aaa;border-bottom:1px solid #1a1a1a">${customerPhone || user?.phone || '—'}</td></tr>
                ...
                <h3 style="font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:.1em;color:#c9a84c;margin-bottom:12px;border-top:1px solid #222;padding-top:20px">Items Ordered (${enrichedItems.length})</h3>
                <table style="width:100%;border-collapse:collapse;background:#111;border:1px solid #222">
                    <tr style="background:#1a1a1a">
                        <th style="padding:10px;text-align:left;color:#888;font-size:10px;text-transform:uppercase;letter-spacing:.1em">Image</th>
                        <th style="padding:10px;text-align:left;color:#888;font-size:10px;text-transform:uppercase;letter-spacing:.1em">Product</th>
                        <th style="padding:10px;text-align:center;color:#888;font-size:10px;text-transform:uppercase;letter-spacing:.1em">Qty</th>
                        <th style="padding:10px;text-align:right;color:#888;font-size:10px;text-transform:uppercase;letter-spacing:.1em">Total</th>
                    </tr>
                    ${itemsAdminHtml}
                </table>

                <div style="margin-top:20px;padding:14px 18px;background:#111;border-left:4px solid #c9a84c">
                    <a href="https://dashboard.stripe.com/payments/${paymentIntentId}" style="color:#c9a84c;font-size:13px;font-weight:700">→ Verify on Stripe Dashboard</a>
                </div>
                </div>
                </div>`
            });
        ...
        // ── CUSTOMER EMAIL ────────────────────────────────────────
        const itemsUserHtml = enrichedItems.map(i => {
            const imgCell = i.image
                ? `<img src="${proxyImg(i.image)}" width="65" height="65" style="object-fit:cover;display:block;border:1px solid #eee;border-radius:6px">`
        ...
        // Priority: metadata email > DB user email > Stripe customer email
        const emailTo = (userEmail && userEmail.includes('@') ? userEmail : '') 
            || (user?.email && user.email.includes('@') ? user.email : '')
            || (customerEmail && customerEmail.includes('@') ? customerEmail : '')
            || '';
        console.log('[webhook] emailTo:', emailTo, '| userEmail:', userEmail, '| user?.email:', user?.email, '| customerEmail:', customerEmail);
        if (emailTo) {
            try {
                await resend.emails.send({
                    from: process.env.NT_EMAIL || 'Mova99 <support@mova99.com>',
                    ...
                    <div style="background:white;padding:32px;border:1px solid #eee">
                        ...
                        <p style="font-size:14px;color:#555;line-height:1.7;margin-bottom:24px">Thank you for your order! Your payment has been received and we are now processing your items. You will receive a shipping confirmation with tracking details once dispatched.</p>
                        <div style="background:#f8f8f8;border-left:4px solid #c9a84c;padding:16px 20px;margin-bottom:24px">
                            <table style="width:100%;border-collapse:collapse;font-size:13px">
                                <tr><td style="color:#888;padding:3px 0">Order Date & Time</td><td style="text-align:right;font-weight:700">${new Date().toLocaleString('en-US',{month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit',hour12:true,timeZone:'America/New_York'})} EST</td></tr>
                                <tr><td style="color:#888;padding:3px 0">Payment Status</td><td style="text-align:right;color:#22c55e;font-weight:700">✓ Payment Received</td></tr>
                                ...
                                <tr><td style="color:#888;padding:3px 0">Estimated Delivery</td><td style="text-align:right;font-weight:700">6-13 Business Days</td></tr>
                            </table>
                        </div>
                        ...
                        <a href="https://www.mova99.com/dashboard#orders" style="display:inline-block;background:#0a0a0a;color:white;padding:14px 28px;text-decoration:none;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.12em">Track My Order →</a>
                    </div>
