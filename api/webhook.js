...
export default async function handler(req, res) {
    ...
    try {
        ...
        // ── Auto-update referrer's 5% balance ─────────────────────
        try {
            if (userId) {
                ...
                if (buyer && buyer.referredBy) {
                    ...
                    console.log('[webhook] Referral commission $' + commission + ' added to code ' + buyer.referredBy);
                }
            }
        } catch(e) { console.error('[webhook] Referral balance update error:', e.message); }
...
<tr><td style="padding:10px 14px;color:#888;width:160px;border-bottom:1px solid #1a1a1a">Order ID (MongoDB)</td><td style="padding:10px 14px;font-weight:700;color:#fafafa;border-bottom:1px solid #1a1a1a;font-family:monospace">${orderId}</td></tr>
        ...
        // Priority: Stripe customer email (what they typed at checkout) > DB email > metadata email
        const emailTo = (customerEmail && customerEmail.includes('@') ? customerEmail : '')
            ...
            || '';
        console.log('[webhook] emailTo:', emailTo, '| customerEmail:', customerEmail, '| user?.email:', user?.email, '| userEmail:', userEmail);
        if (emailTo) {
            try {
                await resend.emails.send({
                    from: process.env.NT_EMAIL || 'Mova99 <support@mova99.com>',
