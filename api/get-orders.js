    ...
    try {
        ...
        for (const order of userOrders) {
                ...
                try {
                    ...
                    for (const item of (order.items || [])) {
                        if (!item.name || item.name === 'undefined') {
                            try {
                                const r = await axios.get('https://developers.cjdropshipping.com/api2.0/v1/product/variant/query', { headers: { 'CJ-Access-Token': process.env.PRODUCTS_API_KEY }, params: { pid: item.pid } });
            ...
            if (order.trackingNumber && order.status === 'shipped') {
                try {
                    const trackRes = await axios.get('https://developers.cjdropshipping.com/api2.0/v1/logistic/trackInfo', { headers: { 'CJ-Access-Token': process.env.PRODUCTS_API_KEY }, params: { trackNumber: order.trackingNumber } });
